...
コース: 北嶺競馬場〈府中モデル〉左回り 芝2083.1m・最終直線525.9m・高低差2.7m（8レーン/LANE_W2.4）。
        ...
        ★回り方向の罠: three.js（y-up）の画面では **heading減少の回り（dir:-1・outSign=+1）が
        左回りに見える**（x-z数学平面の反時計回り＝画面では時計回り。凡例逆転）。府中モデル=左回りは
        dir:-1 で定義する。dir:+1 で組むと「内/外ボタンとコースが逆」になる（実プレイ指摘で発覚・修正済み）。
        外向き法線は outN(heading)/OUT_SGN=Course.outSign に一元化。
